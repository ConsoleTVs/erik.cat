...
to power all the async operations and therefore, this library focuses on handling Solid
JS's reactivity system and provides a familiar API.
...
const App = () => {
  ...
  return (
    <TurboContext value={configuration}>
      <MyApp />
    </TurboContext>
  )
}
